...
class Boardgame:
    ...
    def jouer_carte(self, carte):
        ...
        while True:
            if self.emplacements[emplacement] is None and emplacement in range(0, 6):
                ...
                print(f"La carte {carte} a été jouée sur l'emplacement {emplacement}.")
                return True
            else:
    ...
    def fight_between(self, boardgame2:Boardgame, entity1, entity2):
        """_summary_

        Args:
            boardgame2 (Boardgame): Correspond au plateau de jeu de l'user attaqué. De type class(BoardGame).
            entity1 (Card): Correspond à une carte de type class(Card) de "emplacements" du boardgame1
            entity2 (Card or User): Correspond à une carte de type Card de "emplacements" du boardgame2 OU à l'utilisateur de type class(User)
        Returns:
            [result1, result2] ([bool, bool]): Retourne une liste de deux entités de type bool. le premier bool correspond
                                            à survie ou non des deux entités paramètre.
        """
        result1, result2 = True, True
        entity2.hp - entity1.hit
        if type(entity2) != User:
            ...
            result2 = False                ### Si ce IF est déclenché, cela signifie la victoire d'un des deux joueurs.
                                        ###    Il faudra donc penser à ajouter une méthode victoire peut être dirctement ici ? 
